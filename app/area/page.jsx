  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = 
        await 
        axios.get('http://localhost:3001/api/area/getall', 
        { headers: 
            ...
            {areaData && areaData.map((data) => (
              <tr key={data.id}>
                ...
                <td className='py-3 px-6 hover-bg-sky-500 cursor-pointer duration-300 hover:scale-90 flex justify-center items-center'>
                  <Link className='hover:text-sky-400 transition-colors p-2' href={`/updatearea/${data.id}`}>
                    <AiOutlineEdit />
                  </Link>
                  <button className='hover:text-sky-400 transition-colors p-2' onClick={() => deleteArea(data.id, data.city)}>
                    <MdDeleteOutline />
                  </button>
                </td>
              </tr>
